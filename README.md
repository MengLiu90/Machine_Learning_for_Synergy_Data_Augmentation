...
# Datasets
The file original_synergy_data.tgz provides the original synergy data used for classification. To reduce learning time and memory consumption, we randomly sampled a subset from the entire augmented dataset. It should be noticed that only training set related augmented data can be sampled from and au The complete augmented data can be accessed through https://lsu.box.com/v/data-augmentation.
## Cross-validation
